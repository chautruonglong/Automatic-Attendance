...
class ResidualDenseBlock_5C(Module):
    ...
    def forward(self, x):
        ...
        x2 = self.lrelu(self.conv2(cat((x, x1), 1)))
        x3 = self.lrelu(self.conv3(cat((x, x1, x2), 1)))
        x4 = self.lrelu(self.conv4(cat((x, x1, x2, x3), 1)))
...
class RRDBNet(Module):
    ...
    def forward(self, x):
        ...
        fea = self.lrelu(self.upconv1(interpolate(fea, scale_factor=2, mode='nearest')))
        fea = self.lrelu(self.upconv2(interpolate(fea, scale_factor=2, mode='nearest')))
